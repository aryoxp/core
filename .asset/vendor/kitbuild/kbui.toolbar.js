...
class NodeCreationTool extends KitBuildToolbarTool {
  ...
  show(displayed = true) {
    let toolbartool = $('#' + this.canvas.canvasId + "-" + KitBuildToolbar.NODE_CREATE);
    if (displayed) toolbartool.removeClass('d-none');
    else toolbartool.addClass('d-none');
    return this;
  }
  ...
  control() {
    let controlHtml = 
      `<div class="btn-group btn-group-sm">
        ...
        <div id="dd-menu-compare-switches" class="dropdown-menu p-2" style="min-width:0; z-index:1">
...
class KitBuildToolbar {
  ...
  render() { // render tools' controls on toolbar
    ...
    tools.forEach((t, k, map) => {
      let toolControls = '';
      toolControls += `<span id="${this.canvas.canvasId}-${k}" ${t.settings.visible ? '' : 'class="d-none"'}>`;
