class KitBuild {
  static openConceptMap(cmid) {
    ...
    return this.ajax.get(`m/x/kb/kitBuildApi/openConceptMap/${cmid}`)
  }
  static openKitMap(kid) {
    if (!kid) throw new Error(`Invalid Kit Map ID: ${kid}`);
    this.ajax = Core.instance().ajax()
    return this.ajax.get(`m/x/kb/kitBuildApi/openKitMap/${kid}`)
  }
  ...
  static openLearnerMap(lmid) {
    ...
    this.ajax = Core.instance().ajax()
    return this.ajax.get(`m/x/kb/kitBuildApi/openLearnerMap/${lmid}`)
  }
