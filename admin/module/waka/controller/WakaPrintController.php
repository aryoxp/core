...
class WakaPrintController extends CoreModuleController {
  ...
  public function krs() {
    ...
    $matakuliahs = $service->getListMatakuliahKRS($nrm, $semester, $semesterke);
    $service = new AkademikService();
    $krs = $service->getKRS($nrm, $semesterke, $semester);
    $service = new MahasiswaService();
    $mahasiswa = $service->getMahasiswa($nrm);
    $this->ui->useScript('js/print-krs.js');
    $this->ui->view('head.php', null, CoreView::CORE);
    $this->ui->view('print-krs.php', 
      ['matakuliahs' => $matakuliahs, 'mahasiswa' => $mahasiswa, 'krs' => $krs]
    );
  }
  public function bapresensi() {
    try {
      ...
      $result = $service->getMahasiswaKelas($tahun, $semester, $kdmk, $kurikulum, $nama, 1, 10000);
      $mahasiswas = $result->mahasiswa;
      $service = new MatakuliahService();
      $matakuliah = $service->getMatakuliah($kdmk, $kurikulum);

      $kelas = new stdClass;
      $kelas->tahun = $tahun;
      $kelas->semester = $semester;
      $kelas->kdmk = $kdmk;
      $kelas->kurikulum = $kurikulum;
      $kelas->nama = $nama;

      // Begin MPDF
      require_once('.shared/lib/mpdf/autoload.php');
      $html = $this->ui->view('print-bapresensi.php', ['kelas'=>$kelas, 'mahasiswas'=>$mahasiswas, 'matakuliah'=>$matakuliah], CoreView::RETURN);
      // echo $html; return;
  ...
  public function bapresensiuts() {
    try {
      ...
      // Begin MPDF
      require_once('.shared/lib/mpdf/autoload.php');
      $html = $this->ui->view('print-bapresensiuts.php', ['kelas'=>$kelas, 'mahasiswas'=>$mahasiswas, 'matakuliah'=>$matakuliah], CoreView::RETURN);
      // echo $html; return;
      $defaultConfig = (new Mpdf\Config\ConfigVariables())->getDefaults();
      $fontDirs = $defaultConfig['fontDir'];
      $defaultFontConfig = (new Mpdf\Config\FontVariables())->getDefaults();
  ...
  public function bapresensiuas() {
    try {
      ...
      // Begin MPDF
      require_once('.shared/lib/mpdf/autoload.php');
      $html = $this->ui->view('print-bapresensiuas.php', ['kelas'=>$kelas, 'mahasiswas'=>$mahasiswas, 'matakuliah'=>$matakuliah], CoreView::RETURN);
      // echo $html; return;
      $defaultConfig = (new Mpdf\Config\ConfigVariables())->getDefaults();
      $fontDirs = $defaultConfig['fontDir'];
      $defaultFontConfig = (new Mpdf\Config\FontVariables())->getDefaults();
  ...
  public function khs() {
    ...
    $semesterke = $this->postv('semesterke');
    $service = new AkademikService();
    $matakuliahs = $service->getListMatakuliahKHS($nrm, $semester, $semesterke);
    $service = new AkademikService();
    $khs = $service->getKRS($nrm, $semesterke, $semester);
    $service = new MahasiswaService();
    $mahasiswa = $service->getMahasiswaWithPA($nrm);
    $this->ui->useScript('js/print-khs.js');
    $this->ui->view('head.php', null, CoreView::CORE);
    $this->ui->view('print-khs.php', 
      ['matakuliahs' => $matakuliahs, 'mahasiswa' => $mahasiswa, 'tahun' => $khs->tahun, 'semester' => $semester]
    );
  }
  public function khstahunakademik() {
    $this->ui->useStyle('css/print.css');
    $nrm = $this->postv('nrm');
    $tahun = $this->postv('tahun');
    $semester = $this->postv('semester');
    $service = new AkademikService();
    $matakuliahs = $service->getListMatakuliahKHSTahunAkademik($nrm, $tahun, $semester);
    $service = new MahasiswaService();
    $mahasiswa = $service->getMahasiswaWithPA($nrm);
    $data = ['matakuliahs' => $matakuliahs, 'mahasiswa' => $mahasiswa, 'tahun' => $tahun, 'semester' => $semester];
    $this->ui->useScript('js/print-khs.js');
    $this->ui->view('head.php', null, CoreView::CORE);
    $this->ui->view('print-khs.php', $data);
  }
  public function khstahunakademikselected() {
    $this->ui->useStyle('css/print.css');
    ...
    foreach($nrms as $nrm) {
      $service = new AkademikService();
      $matakuliahs = $service->getListMatakuliahKHSTahunAkademik($nrm, $tahun, $semester);
      if (count($matakuliahs) == 0) continue;
      $service = new MahasiswaService();
      $mahasiswa = $service->getMahasiswaWithPA($nrm);
      $data = ['matakuliahs' => $matakuliahs, 'mahasiswa' => $mahasiswa, 'tahun' => $tahun, 'semester' => $semester];
      if ($i > 0) $data['pb'] = true;
  ...
  public function transkrip() {
    ...
    $nrm = $this->postv('nrm');
    $service = new AkademikService();
    $matakuliahs = $service->getTranskripNilaiPrint($nrm);
    $service = new MahasiswaService();
    $mahasiswa = $service->getMahasiswaWithPA($nrm);
    $data = ['matakuliahs' => $matakuliahs, 'mahasiswa' => $mahasiswa];
    // $this->ui->view('head.php', null, CoreView::CORE);
    $html = "hello";
    $html = $this->ui->view('print-transkrip.php', $data, CoreView::RETURN);
    // $this->ui->view('print-transkrip.php', $data); return;
    // Begin MPDF
    require_once('.shared/lib/mpdf/autoload.php');
    // $html = $this->ui->view('print-bapresensi.php', 
    // ['kelas'=>$kelas, 'mahasiswas'=>$mahasiswas, 'matakuliah'=>$matakuliah], CoreView::RETURN);
    // echo $html; return;
    $defaultConfig = (new Mpdf\Config\ConfigVariables())->getDefaults();
    $fontDirs = $defaultConfig['fontDir'];
    $defaultFontConfig = (new Mpdf\Config\FontVariables())->getDefaults();
  ...
  public function transkripakhir() {
    ...
    foreach($nrms as $nrm) {
      $service = new AkademikService();
      $matakuliahs = $service->getTranskripNilaiAkhirPrint($nrm);
      $akademik = $service->getDataAkademikMahasiswa($nrm);
      $service = new MahasiswaService();
      $mahasiswa = $service->getMahasiswaWithPA($nrm);
      $data = ['matakuliahs' => $matakuliahs, 'mahasiswa' => $mahasiswa, 'akademik' => $akademik];
      // $this->ui->view('head.php', null, CoreView::CORE);
      if ($i > 0) $data['pb'] = true;
      $html .= $this->ui->view('print-transkrip-akhir.php', $data, CoreView::RETURN);
      $i++;
      // break;
    }
    // $this->ui->view('print-transkrip.php', $data); return;
    ...
    $css = file_get_contents('kv-mpdf-bootstrap.min.css');
    $css2 = file_get_contents('print-transkrip.css');
    $mpdf->defaultheaderline = 0;
    $mpdf->defaultfooterline = 0;
    // $mpdf->setFooter('<div class="text-primary" style="font-family:firasanscondensed;font-style:normal;font-weight:normal">Halaman {PAGENO}/{nb}</div>');
    // $mpdf = new \Mpdf\Mpdf();
  ...
  public function ijazah() {
    ...
    foreach($nrms as $nrm) {
      ...
      // $this->ui->view('head.php', null, CoreView::CORE);
      if ($i > 0) $data['pb'] = true;
      $html .= $this->ui->view('print-ijazah.php', $data, CoreView::RETURN);
      $i++;
      // break;
    }
    // $this->ui->view('print-transkrip.php', $data); return;
    ...
    $mpdf = new \Mpdf\Mpdf([
      ...
      'fontdata' => $fontData + [ // lowercase letters only in font key
        ...
        'neuton' => [
          'R' => 'Neuton-Regular.ttf'
        ],
        'sharetechmono' => [
          'R' => 'ShareTechMono-Regular.ttf'
        ]
      ],
      'default_font' => 'firasans'
    ]);
    $css = file_get_contents('kv-mpdf-bootstrap.min.css');
    $css2 = file_get_contents('print-ijazah.css');
    $mpdf->defaultheaderline = 0;
    $mpdf->defaultfooterline = 0;
    ...
    $mpdf->WriteHTML($css,1);
    $mpdf->WriteHTML($css2,1);
    $mpdf->WriteHTML('<watermarktext content="DRAFT" alpha="0.1" />');
    $mpdf->WriteHTML($html);
    $mpdf->Output();
  }
