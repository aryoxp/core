$(() => {
  ...
  class App {
    ...
    handleEvent() {
      $('#form-search').on('submit', e => {
        ...
        this.ajax.post(`m/x/wmon/laporanApi/getLaporanPengeluaranKerumahtanggaan/${page}/${perpage}`, {
          tanggal: tanggal
      ...
      $('.bt-print').on('click', (e) => {
        ...
        e.stopPropagation();
        let tanggal = $('#input-tanggal').data().datepicker.getFormattedDate('yyyy-mm-dd');
        let location = (Core.configuration.get('baseurl') + `m/x/wmon/print/laporanpengeluarankerumahtanggaan/${tanggal}`);
  ...
  App.populateTransaksi = (transaksis) => {
    ...
    transaksis.forEach(transaksi => {
      ...
      html += `<div class="transaksi-item list-item py-1 px-2 border-bottom" data-nrm="${transaksi.nrm}">`
      html += `<div class="d-flex justify-content-between">`;
      html += `<span class="d-flex align-items-center text-nowrap text-truncate">`
      html += `${transaksi.pengeluaran}`
      ...
      html += `<span class="badge rounded border-1 bg-primary text-light ms-2">${transaksi.kas}</span>`
      html += `</span>`;
      html += `</div>`;
      html += `<div class="text-muted me-4">${transaksi.keterangan}</div>`;
      html += `</div>`;
    });
