  ...
  class App {
    ...
    handleEvent() {
      $('#form-search-pengeluaran').on('submit', e => {
        ...
        this.ajax.post(`m/x/wmon/transaksiApi/searchPengeluaran/${page}/${perpage}`, {
          year: year,
          month: month
        }).then(results => { console.log(results);
          let transaksis = results.transaksi;
          let count = parseInt(results.count);
          App.populateTransaksiPengeluaran(transaksis);
          if (App.pagination) {
      ...
      $('#form-pembayaran').on('click', '.bt-bayar', (e) => {
        ...
        (new CoreConfirm(`Catat pengeluaran <span class="text-danger">${akundebit}</span> sebesar ${App.format(nominal)}?`)).positive((e) => {
          this.ajax.post('m/x/wmon/transaksiApi/catatTransaksiPengeluaran'
            ...
            (new CoreInfo(`Pengeluaran <span class="text-danger">${akundebit}</span> telah dicatat.`)).show();
