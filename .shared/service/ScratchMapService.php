...
class ScratchMapService extends CoreService {

  function insert($userid, $type, $title, $data) {
    ...
    $db = self::instance();
    try {
      $qb = QB::instance('scratchmap')->insert($insert);
      $db->query($qb->get());
      $id = $db->getInsertId();
      return $id;
    } catch (Exception $ex) {
  ...
  function getLearnerMapsOfUserId($userid) {
    try {
      $db = self::instance();
      $qb = QB::instance('scratchmap l');
      $qb->select()
        ->where('l.userid', QB::esc($userid))
        ->orderBy('l.userid')
        ->orderBy('l.created');
      $scratchmaps = $db->query($qb->get());
      return $scratchmaps;
    } catch (Exception $ex) {
      throw CoreError::instance($ex->getMessage());
    }
