...
class CoreAuth extends CoreService {
  ...
  public static function isMenuAuthorized($app, $key = null) {
    if ($app instanceof CoreMenu) {
      $menu = $app;
      if (!property_exists($menu, 'id')) return true;
      if (property_exists($menu, 'type') && $menu->type == "public") return true;
      return ($menu->id == $key); // return true;
    }
    $authMenu = $_SESSION['authmenu'] ?? [];
    $authFunction = $_SESSION['authfunction'] ?? []; 
    if ($authMenu) {
      $mids = $authMenu[$app] ?? [];
      return in_array($key, $mids);
    } 
    return false; 
  }
